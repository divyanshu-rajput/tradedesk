...
interface AppState {
  market: {
    symbols: Record<string, {        // keyed by symbol, e.g. "BTCUSDT"
      price: number;
...
connect$ = createEffect(() =>
  this.actions$.pipe(
    ...
    switchMap(() =>
      this.feed.stream$().pipe(           // feed = live socket OR demo stream (DI token)
...
export const selectPnl = createSelector(
  selectHoldings, selectMarketSymbols,
  (holdings, symbols) => holdings.map(h => computePnl(h, symbols[h.symbol])),
);
```
...
export class PriceFlashDirective {
  ...
  @Input('priceFlash') set value(curr: number) { /* compare to previous, addClass up/down */ }
...
export const appConfig: ApplicationConfig = {
  providers: [
    ...
    { provide: MARKET_FEED, useFactory: feedFactory, deps: [/* env */] },
  ],
};
```
